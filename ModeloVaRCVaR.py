...
# Selector de acción
stock_seleccionado = 'NVDA' # st.selectbox("Selecciona una acción", stocks_lista)
...
# Mostrar la gráfica
    ...
    st.write("De acuerdo con los resultados obtenidos en las gráfica y las tabla anteriores, en general, nuestras estimaciones son precisas. Sin embargo, se observa que las métricas de VaR histórico y paramétrico, ademas del var con el metodo alternativo, al 95% de confianza presentan un mayor margen de error. Esto podría explicarse por el hecho de que se está utilizando un modelo basado en una distribución normal, cuando en realidad los rendimientos podrían seguir una distribución con colas más pesadas, lo cual es típico en los mercados financieros.")
